...
public class EditListDetailServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try{
			String[] selectedUnits = request.getParameterValues("allUnitsToAdd");
			List<Unit> selectedUnitsInList = new ArrayList<Unit>();
			
			for(int i = 0; i < selectedUnits.length; i++) {
				System.out.println(selectedUnits[i]);
				Unit c = uh.searchForUnitById(Integer.parseInt(selectedUnits[i]));
				selectedUnitsInList.add(c);
			}
			listToUpdate.setRoster(selectedUnitsInList);
		}
